...
C_FOOT = RGBColor(0x66, 0x66, 0x66)  # 각주 색상 — S2·S3·S4 통일 (#666666)
...
def build_slide_4(slide):
    ...
    sec1_tb = add_textbox(
        ...
        safe_width, Inches(0.22),
        "① 영역별 효과 (업계 일반 보정 추정)",
        font_size=11, bold=True, color=C_NAVY,
    )

    # 산정 방식 메모 (2줄, 9pt) — 행별 괄호 병기와 함께 전체 방법론 설명
    method_note_y = Inches(1.32)
    method_note_h = Inches(0.32)
    mn_tb = slide.shapes.add_textbox(
        safe_left, method_note_y, safe_width, method_note_h,
    )
    mn_tf = mn_tb.text_frame
    mn_tf.word_wrap = True
    mn_tf.margin_left = Inches(0)
    mn_tf.margin_right = Inches(0)
    mn_tf.margin_top = Inches(0)
    mn_tf.margin_bottom = Inches(0)
    mn_lines = [
        ("산정 베이스", "GitHub Research +55%  ·  METR -19%  ·  Faros AI +91%  ·  BMW SPACE 5축"),
        ("제약 계수", "규정 준수 (ASPICE · ISO 26262)  ·  학습 데이터 부족  ·  Target HW 제약  ·  추적성 / 회귀검증"),
    ]
    for i, (label, text) in enumerate(mn_lines):
        if i == 0:
            p = mn_tf.paragraphs[0]
        else:
            p = mn_tf.add_paragraph()
    ...
    cards_data = [
        ...
        },
        {
            ...
            "detail": "작업 처리량 +21%",
            "accent": C_RED,
            "reasons_title": "▶ 리뷰 부담 급증 원인",
            "reasons": [
                "AI 코드 생성량 급증 → PR 수 2배 (+98%)",
                "코드 중복 4배 (GitClear 2025) → MISRA C:2025 중복·복잡도 규칙 위반 확인 부담",
                "자동차 양산: ASPICE · ISO 26262 규정 준수 검증은 여전히 사람 몫",
            ],
        },
    ]
    ...
    for i, card in enumerate(cards_data):
        ...
        # 원인 블록 (있을 때만) — 왜 이 수치가 나왔는지 해설
        if card.get("reasons"):
            # 얇은 구분선
            sep2 = slide.shapes.add_shape(
                MSO_SHAPE.RECTANGLE,
                inner_x, y + Inches(0.04),
                inner_w, Emu(9525),
            )
            sep2.fill.solid()
            sep2.fill.fore_color.rgb = C_LGRAY
            sep2.line.fill.background()
            y += Inches(0.12)

            reasons_h = Inches(1.10)
            rtb = slide.shapes.add_textbox(inner_x, y, inner_w, reasons_h)
            rtf = rtb.text_frame
            rtf.word_wrap = True
            ...
            rp0.alignment = PP_ALIGN.LEFT
            rp0.space_after = Pt(2)
            rr_t = rp0.add_run()
            rr_t.text = card["reasons_title"]
            rr_t.font.size = Pt(10)
            rr_t.font.bold = True
            rr_t.font.color.rgb = card["accent"]

            # 원인 bullet
            for reason in card["reasons"]:
                rp = rtf.add_paragraph()
                rp.alignment = PP_ALIGN.LEFT
                rp.space_before = Pt(1)
                rp.space_after = Pt(0)
                rp.line_spacing = 1.15
                rr_b = rp.add_run()
                rr_b.text = "• "
                rr_b.font.size = Pt(9)
                rr_b.font.bold = True
                rr_b.font.color.rgb = card["accent"]
                rr_r = rp.add_run()
                rr_r.text = reason
                rr_r.font.size = Pt(9)
                rr_r.font.color.rgb = C_INK
    ...
    # SPACE, Faros AI 는 S2에서 먼저 해설 — 이 슬라이드에서 중복 각주 생략
    foot_lines = [
